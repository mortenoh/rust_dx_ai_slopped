...
impl App {
    ...
    fn drill_down(&mut self) -> Result<()> {
        if let Some(ou) = self.selected_org_unit().cloned() {
            self.breadcrumb
                .push((ou.id.clone(), ou.display_name.clone()));
    ...
    fn go_up(&mut self) -> Result<()> {
        if self.breadcrumb.pop().is_some() {
            ...
            self.search_query.clear();
            let parent_id = self.current_parent_id.clone();
            self.load_children(parent_id.as_deref())?;
        }
        Ok(())
    }
...
fn render_list(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let list = List::new(items)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(format!(" {} items ", app.filtered_units.len())),
        )
        .highlight_style(
            Style::default()
                .bg(Color::DarkGray)
